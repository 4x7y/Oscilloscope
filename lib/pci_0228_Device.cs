...
namespace Jungo.pci_0228_lib
{
     ...
     * function type */
    public delegate void USER_EVENT_CALLBACK(ref WD_EVENT pEvent, PCI_0228_Device dev);
    /* PCI diagnostics interrupt handler function type */
    public delegate void USER_INTERRUPT_CALLBACK(PCI_0228_Device device);


    public class PCI_0228_Device
    {
        ...
        private PCI_0228_Regs m_regs;

#region " constructors " 
        /* constructors & destructors */
        internal protected PCI_0228_Device(WD_PCI_SLOT slot): this(0, 0, slot){}

        internal protected PCI_0228_Device(DWORD dwVendorId, DWORD dwDeviceId,
            WD_PCI_SLOT slot)
        {
            ...
            m_eventHandler = new EVENT_HANDLER_DOTNET(PCI_0228_EventHandler);
            m_regs = new PCI_0228_Regs();
...
#region " utilities " 
        ...
        {
            m_sDeviceLongDesc = string.Format("PCI_0228 Device: Vendor ID 0x{0:X}, " 
        ...
        private bool DeviceValidate()
        {
            ...
            Log.TraceLog("PCI_0228_Device.DeviceValidate: Device does not have "
...
#region " Device Open/Close " 
        ...
        public virtual DWORD Open()
        {
            ...
            {
                Log.ErrLog("PCI_0228_Device.Open: Failed retrieving the " 
            ...
            if ((DWORD)wdc_err.WD_STATUS_SUCCESS != dwStatus)
            {
                Log.ErrLog("PCI_0228_Device.Open: Failed opening a " +
            ...
            Log.TraceLog("PCI_0228_Device.Open: Opened a PCI device " + 
                this.ToString(false));
...
Error:    
        ...
        public virtual bool Close()
        {
            ...
            if (Handle == IntPtr.Zero)
            {
                Log.ErrLog("PCI_0228_Device.Close: Error - NULL " 
                    + "device handle");
                return false;
            }
            ...
            if ((DWORD)wdc_err.WD_STATUS_SUCCESS != dwStatus)
            {
                Log.ErrLog("PCI_0228_Device.Close: Failed closing a "
            ...
            {
                Log.TraceLog("PCI_0228_Device.Close: " +
...
#region " Interrupts "
        ...
        public DWORD DisableInterrupts()
        {
            ...
            {
                Log.ErrLog("PCI_0228_Device.DisableInterrupts: interrupts are already disabled... " +
                    "(" + this.ToString(false) + ")" );
                return (DWORD)wdc_err.WD_OPERATION_ALREADY_DONE;
            }
            ...
            if (dwStatus != (DWORD)wdc_err.WD_STATUS_SUCCESS)
            {
                Log.ErrLog("PCI_0228_Device.DisableInterrupts: Failed to" +
            ...
            {
                Log.TraceLog("PCI_0228_Device.DisableInterrupts: Interrupts are disabled" +
                    "(" + this.ToString(false) + ")");
            }
            ...
            /* private methods */
        private void PCI_0228_IntHandler(IntPtr pDev)
        {
            wdcDevice.Int =
...
#region " Events"
        ...
        public DWORD EventRegister(USER_EVENT_CALLBACK userEventHandler)
        {
            ...
            {
                Log.ErrLog("PCI_0228_Device.EventRegister: user callback is "
                    + "null");
                return (DWORD)wdc_err.WD_INVALID_PARAMETER;
            }
            ...
            {
                Log.ErrLog("PCI_0228_Device.EventRegister: Events are already "
                    + "registered ...");
                return (DWORD)wdc_err.WD_OPERATION_ALREADY_DONE;
            }
            ...
            if ((DWORD)wdc_err.WD_STATUS_SUCCESS != dwStatus)
            {
                Log.ErrLog("PCI_0228_Device.EventRegister: Failed to register "
            ...
            {
                Log.TraceLog("PCI_0228_Device.EventRegister: events are " +
        ...
        public DWORD EventUnregister()
        {
            ...
            if (!wdc_lib_decl.WDC_EventIsRegistered(Handle))
            {
                Log.ErrLog("PCI_0228_Device.EventUnregister: No events " +
            ...
            if ((DWORD)wdc_err.WD_STATUS_SUCCESS != dwStatus)
            {
                Log.ErrLog("PCI_0228_Device.EventUnregister: Failed to " +
            ...
            {
                Log.TraceLog("PCI_0228_Device.EventUnregister: Unregistered " +
